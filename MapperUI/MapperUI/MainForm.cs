...
namespace MapperUI
{
    public partial class MainForm : Form
    {
        ...
        {
            using var dlg = new OpenFileDialog { Filter = "XML|*.xml|All|*.*", Title = "Open VueOne Control.xml" };
        ...
        async Task LoadAndValidateAsync(string path)
        {
            ...
            try
            {
                ...
                foreach (var rule in RuleEngine.GetAllRules()) AddRuleRow(rule);
                ...
                lblValidationStatus.Text = ok ? "PASSED" : "FAILED";
                lblValidationStatus.ForeColor = ok ? Color.Green : Color.Red;
        ...
        void AddRuleRow(MappingRuleEntry rule)
        {
            int idx = dgvMappingRules.Rows.Add(
                ...
                rule.IsSection ? "" : (rule.IsImplemented ? Tick : Cross));
        ...
        static ComponentValidationRow Validate(VueOneComponent comp, ComponentValidator validator, MapperConfig cfg)
        {
            ...
            string tName = string.IsNullOrEmpty(tPath)
                ? "No template found (discarded for this phase)" : Path.GetFileName(tPath);
