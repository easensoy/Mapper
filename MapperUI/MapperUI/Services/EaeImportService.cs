    ...
    public static class EaeImportService
    {
        ...
        public static EaeImportResult Import(MapperConfig cfg, List<VueOneComponent> components)
        {
            ...
            foreach (var basic in neededBasics.OrderBy(b => b))
            {
                ...
                if (exportFile != null)
                {
                    result.ImportFiles.Add(exportFile);
                    MapperLogger.Info($"[Import] Staged Basic: {basic} → {Path.GetFileName(exportFile)}");
                }
                step++;
            }
            ...
            foreach (var cat in neededCats.OrderBy(c => c))
            {
                ...
                {
                    result.ImportFiles.Add(exportFile);
                    MapperLogger.Info($"[Import] Staged CAT: {cat} → {Path.GetFileName(exportFile)}");
                }
                step++;
            }
            ...
            if (result.ImportFiles.Count == 0)
            {
                result.Warnings.Add("No importable templates found for the loaded components.");
                return result;
            }

            Process.Start("explorer.exe", stagingDir);
            result.Success = true;
            return result;
        }
