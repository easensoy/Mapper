...
namespace MapperUI.Services
{
    public class MapperService
    {
            ...
            string deploySubDir)
        {
            ...
            void EnsureCompile(string rel)
            {
                if (AlreadyPresent(compileGroup, ns + "Compile", rel)) return;
                compileGroup.Add(new XElement(ns + "Compile", new XAttribute("Include", rel)));
                MapperLogger.Info($"[dfbproj]  + Compile  {rel}");
                adds++;
            }

            void EnsureNone(string rel)
            {
                if (AlreadyPresent(noneGroup, ns + "None", rel)) return;
                noneGroup.Add(new XElement(ns + "None", new XAttribute("Include", rel)));
                MapperLogger.Info($"[dfbproj]  + None     {rel}");
                adds++;
            }

            // Register .fbt as Compile
            EnsureCompile($@"{subfolderName}\{generatedFB.FbtFile}");

            // Register companion files as None
            EnsureNone($@"{subfolderName}\{generatedFB.CompositeFile}");
            EnsureNone($@"{subfolderName}\{generatedFB.DocFile}");
            EnsureNone($@"{subfolderName}\{generatedFB.MetaFile}");
