...
namespace CodeGen.Services
{
    ...
    internal static class SwivelCatPatcher
    {
        ...
        {
            var fbt = FindDeployedFbt(scope.Root, scope.Manifest.FbtOf("centreHomeCat"));
        ...
        internal static void EnsureSensorBoolReadEvent(FbtEditScope scope, DeployResult result)
            => EditDeployedFbt(scope, scope.Manifest.FbtOf("sensorCat"), "Sensor_Bool_CAT RD event inject failed", result,
                (doc, root, ns, fbt) =>
            {
        ...
        private static void EditSwivelCore(FbtEditScope scope, string failNote, DeployResult result,
            ...
            => EditDeployedFbt(scope, scope.Manifest.FbtOf("centreHomeCore"), failNote, result, edit);
        ...
        internal static void PatchSwivelBrakeHome(FbtEditScope scope, int brakeMs, DeployResult result)
        {
            ...
            var ecc = Path.Combine(scope.Root, "IEC61499", scope.Manifest.FbtOf("centreHomeCore"));
            if (!File.Exists(ecc))
            {
                ecc = Directory.EnumerateFiles(Path.Combine(scope.Root, "IEC61499"),
                        scope.Manifest.FbtOf("centreHomeCore"), SearchOption.AllDirectories).FirstOrDefault() ?? string.Empty;
            ...
            var cat = Directory.EnumerateFiles(Path.Combine(scope.Root, "IEC61499"),
                scope.Manifest.FbtOf("centreHomeCat"), SearchOption.AllDirectories).FirstOrDefault();
