...
namespace CodeGen.Translation
{
    public class SystemInjector
    {
            ...
            IoBindings? bindings, out BindingApplicationReport report)
        {
            ...
            var builder = new SyslayBuilder(layerId, ctx.Cfg.Generation.LayerName);
            // WHAT THE READER OF THE GENERATED FILE IS TOLD. It is emitted into every .syslay, so it
            // has to be true of every plant this compiler serves - not of the one it was written for.
            //
            // Its predecessor described a "scope filter" trimming the layout to four named components.
            // No such filter exists: the loop below emits EVERY process the roster places, and a
            // component a recipe cannot reach is REFUSED rather than falling back to (0,0). So the
            // note shipped in every artefact contradicted the code that wrote it, and named one rig's
            // instances in a file the compiler emits for any.
            builder.SetTopComment(
                "Generated by VueOneMapper from a VueOne Control.xml. Every process the deployment " +
                "roster places is emitted on the resource that owns it, and each carries its compiled " +
                "recipe as Parameter values on its own instance (StepType, CmdTargetName, CmdStateArr, " +
                "Wait1Id, Wait1State, NextStep, AltCount, TermCount). A model semantic this compiler " +
                "cannot express is refused before anything is written, so nothing here is an " +
                "approximation of the twin. Do not hand-edit: the project is regenerated whole, and a " +
                "generation that fails leaves the previous one untouched.");
            ...
            foreach (var resource in ctx.Profile.Layout.Resources)
            {
                ...
                foreach (var proc in ctx.ProcessesOn(resource.Plc))
                {
                    ...
                    builder.AddFB(FBIdGenerator.GenerateFBId(proc.ComponentID), instance,
                        ctx.Manifest.ProcessType.Name, ctx.Cfg.Generation.ProjectNamespace, at?.X ?? 0, at?.Y ?? 0, outer);
                    placed.Add(instance);
            ...
            for (int i = 0; i < contents.Actuators.Count; i++)
            {
                ...
                builder.AddFB(FBIdGenerator.GenerateFBId(actuator.ComponentID),
                    displayName, fbType, ctx.Cfg.Generation.ProjectNamespace,
            ...
            for (int i = 0; i < contents.Sensors.Count; i++)
            {
                ...
                builder.AddFB(FBIdGenerator.GenerateFBId(sensor.ComponentID),
                    senDisplayName, ctx.Manifest.SensorType.Name, ctx.Cfg.Generation.ProjectNamespace,
                    sX, sY,
            ...
            if (ctx.InjectedReporters.Count > 0)
            {
                ...
                foreach (var (synthName, synthId) in ctx.InjectedReporterRows)
                {
                    builder.AddFB(FBIdGenerator.GenerateFBId("m262rigsensor-" + synthName),
                        ...
                        ctx.Cfg.Generation.ProjectNamespace, grid.X, synthY,
                        new Dictionary<string, string>
                        {
