...
namespace CodeGen.Services
{
    public static class RuntimeArtifactVerifier
    {
            ...
            string syslayPath, MapperConfig cfg, Action<string>? log = null)
        {
            ...
            XDocument syslayDoc;
            try { syslayDoc = LoadShared(syslayPath); }
            catch { return 0; }

            var syslayById = syslayDoc.Descendants(Ns + "FB")
                .Where(f => !string.IsNullOrWhiteSpace((string?)f.Attribute("ID")))
                .ToDictionary(
                    f => ((string?)f.Attribute("ID") ?? string.Empty).Trim(),
                    f => f,
            ...
            foreach (var sysresFile in Directory.EnumerateFiles(systemDir, "*.sysres", SearchOption.AllDirectories))
            {
                ...
                bool changed = false;
                foreach (var fb in sysresDoc.Descendants(Ns + "FB"))
                {
                    var mapping = ((string?)fb.Attribute("Mapping") ?? string.Empty).Trim();
                    if (mapping.Length == 0 || !syslayById.TryGetValue(mapping, out var source))
                        continue;

                    changed |= SyncAttribute(fb, "Type", (string?)source.Attribute("Type"));
                    changed |= SyncAttribute(fb, "Namespace", (string?)source.Attribute("Namespace"));

                    var oldParams = fb.Elements(Ns + "Parameter")
                        .Select(p => $"{(string?)p.Attribute("Name")}={(string?)p.Attribute("Value")}")
                        .ToArray();
                    var newParams = source.Elements(Ns + "Parameter")
                        .Select(p => $"{(string?)p.Attribute("Name")}={(string?)p.Attribute("Value")}")
                        .ToArray();

                    if (!oldParams.SequenceEqual(newParams, StringComparer.Ordinal))
                    {
                        fb.Elements(Ns + "Parameter").Remove();
                        foreach (var p in source.Elements(Ns + "Parameter"))
                        {
                            fb.Add(new XElement(Ns + "Parameter",
                                new XAttribute("Name", (string?)p.Attribute("Name") ?? string.Empty),
                                new XAttribute("Value", (string?)p.Attribute("Value") ?? string.Empty)));
                        }
                        changed = true;
                        synced++;
                    }
