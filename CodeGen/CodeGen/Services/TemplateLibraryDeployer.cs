        ...
        public static DeployResult DeployUniversalArchitecture(MapperConfig cfg)
        {
            ...
            if (cfg.MqttPublishEnabled)
            {
                ...
                PatchCatMqttPublish(eaeProjectDir, "Sensor_Bool_CAT",
                    ...
                    initSource: "StateHandling.INITO", cfg, result);
                // Bridge enabler: expose Five_State_Actuator_CAT's post-update
                // state event + INT at the CAT boundary so the BX1-side bridge
                // publishers (emitted by SystemLayoutInjector) can wire to
                // <comp>.state_out / <comp>.current_state_to_process from the
                // syslay across resources. Sensor_Bool_CAT already exposes
                // pst_out + Status at its boundary (verified .fbt), so it does
                // not need this patch — the bridge wires reach those names
                // directly. Idempotent.
