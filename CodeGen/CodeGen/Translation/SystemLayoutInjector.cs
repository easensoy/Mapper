            ...
            IoBindings? bindings, MapperConfig? config, out BindingApplicationReport report)
        {
            ...
            if (config != null && config.MqttPublishEnabled)
            {
                ...
                var mqttParams = new Dictionary<string, string>
                {
                    ...
                    // mosquitto's server.crt (generated by openssl into
                    // C:\VueOneMapper\MQTT\certs\). EAE 24.1's MQTT_CONNECTION
                    // defaults to ValidateCert='Server certificate and
                    // hostname' (strict) and a plain mqtt:// broker trips
                    // ReturnCode=100. Providing a CACert path against the
                    // matching mqtts://127.0.0.1:8883 broker satisfies the
                    // strict validator. The cert's CN is 127.0.0.1 so the
                    // hostname check also passes for a localhost broker.
                    ["CACert"] = SyslayBuilder.FormatString(config.MqttCaCertPath),
                };
